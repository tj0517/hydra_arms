...
## Zakres
- [ ] odczyt stanu: `scripts/bl-build-categories.ts`, `HydraTree`, `computeTags`, `IMPORT_OWNED_TAGS` w `scripts/xml-to-baselinker.ts`
- [ ] tj kopiuje plik z serwera: `scp hydra-srv:hydra/xml-integration/hydra-categories.json xml-integration/`. Agent nie łączy się z serwerem
- [ ] skrypt tylko do odczytu (np. `scripts/bl-verify-categories.ts`): porównuje plik z kategoriami w BL (`getInventoryCategories`) i z drzewem z txt, sprawdza istnienie tagów używanych przez import (`auto`, `review`, `flag`, `age_18`, `approved`). Uruchamia tj, bo hook blokuje skrypty
