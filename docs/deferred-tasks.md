...
- 2026-09-22 · review HA-1.04 (runda 2) · hook przepuszcza po `cd scripts`: nazwę bez `.ts` (`npx tsx reset-shop-db`), wzorzec (`reset-shop-*.ts`) i pętlę `for f in *.ts` · zamyka HA-1.05 (guard w samych skryptach), nie łatać wzorcami w hooku
