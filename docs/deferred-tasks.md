...
- 2026-09-23 · HA-1.06 · 3 testy listing.spec.ts nie przechodzą na lokalnej bazie (preegzystujące, nie regresja HA-1.06): `shows product name and price on cards` i `search filters by name` — selektor `a[href^="/sklep/"] h3` nie widoczny w headless (GSAP fade-in nie odpala); `category sidebar visible on desktop` — oczekiwany tekst `WSZYSTKIE PRODUKTY`, faktyczny `WSZYSTKIE` · naprawić w HA-1.07 lub osobnym zadaniu test-fix
