...
void main(){
  ...
  float maskVal = 0.0;
  if (uUseMask > 0.5) {
    vec2 maskUV = vec2(gl_FragCoord.x / uResolution.x, gl_FragCoord.y / uResolution.y);
    maskVal = texture(uMaskTexture, maskUV).r;
    feed += maskVal * 0.4;
  }
  ...
  vec3 color = mix(uColor, uMaskColor, maskVal);
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    const needsReinitKeys = ['antialias', 'liquid', 'noiseAmount', 'maskText'];
    const cfg = { antialias, liquid, noiseAmount, maskText };
    let mustReinit = false;
    if (!threeRef.current) mustReinit = true;
    else if (prevConfigRef.current) {
      for (const k of needsReinitKeys)
        if (prevConfigRef.current[k] !== cfg[k as keyof typeof cfg]) { mustReinit = true; break; }
    }
    if (mustReinit) {
      if (threeRef.current) {
      ...
      if (maskText) {
        ...
        // dummy 1×1 white texture so the sampler is always bound
        const dummy = document.createElement('canvas');
        dummy.width = 1; dummy.height = 1;
        const dctx = dummy.getContext('2d')!;
        dctx.fillStyle = 'white';
        dctx.fillRect(0, 0, 1, 1);
        maskTex = new THREE.CanvasTexture(dummy);
      }

      const drawMask = (w: number, h: number) => {
        if (!maskCanvas || !maskText || !maskTex) return;
        maskCanvas.width = w;
        maskCanvas.height = h;
        const ctx = maskCanvas.getContext('2d')!;
        ctx.fillStyle = 'black';
        ctx.fillRect(0, 0, w, h);
        // size font to fill ~65% of canvas width, centered
        const refSize = h * 0.75;
        ctx.font = `900 ${refSize}px 'Arial Black', 'Helvetica Neue', Arial, sans-serif`;
        const measured = ctx.measureText(maskText).width;
        const targetWidth = w * 0.65;
        const fontSize = refSize * (targetWidth / Math.max(measured, 1));
        ctx.font = `900 ${fontSize}px 'Arial Black', 'Helvetica Neue', Arial, sans-serif`;
        ctx.fillStyle = 'white';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(maskText, w / 2, h * 0.55);
      ...
      const uniforms = {
        ...
        uMaskTexture: { value: maskTex },
        uUseMask: { value: maskText ? 1.0 : 0.0 },
        uMaskColor: { value: new THREE.Color(maskColor ?? color) },
      };
      ...
      const setSize = () => {
        ...
        drawMask(renderer.domElement.width, renderer.domElement.height);
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach(p => {
            ...
            if (effs) effs.forEach((eff: Effect) => { const u = eff.uniforms?.get('uTime'); if (u) u.value = uniforms.uTime.value; });
          });
          composer.render();
      ...
      raf = requestAnimationFrame(animate);
      threeRef.current = { renderer, scene, camera, material, clock, clickIx: 0, uniforms, resizeObserver: ro, raf, quad, timeOffset, composer, touch, liquidEffect, maskTexture: maskTex };
    } else {
      const t = threeRef.current;
      t.uniforms.uShapeType.value = SHAPE_MAP[variant] ?? 0;
      t.uniforms.uPixelSize.value = pixelSize * t.renderer.getPixelRatio();
      t.uniforms.uColor.value.set(color);
      t.uniforms.uMaskColor.value.set(maskColor ?? color);
  ...
  }, [antialias, liquid, noiseAmount, maskText, maskColor, pixelSize, patternScale, patternDensity, enableRipples, rippleIntensityScale, rippleThickness, rippleSpeed, pixelSizeJitter, edgeFade, transparent, liquidStrength, liquidRadius, liquidWobbleSpeed, autoPauseOffscreen, variant, color, speed]);
